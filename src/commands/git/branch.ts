		...
		while (this.canStepsContinue(state)) {
			if (state.counter < 3 || state.reference == null) {
				const result = yield* pickBranchOrTagStep(state, context, {
					placeholder: context => `选择分支${context.showTags ? '或标签' : ''}以从中创建新分支`,
		...
		const step: QuickPickStep<FlagsQuickPickItem<CreateFlags>> = QuickCommand.createConfirmStep(
			...
			[
				FlagsQuickPickItem.create<CreateFlags>(state.flags, [], {
					label: context.title,
					detail: `将从 ${GitReference.toString(state.reference)} 创建名为 ${state.name} 的新分支`,
				}),
				FlagsQuickPickItem.create<CreateFlags>(state.flags, ['--switch'], {
					label: `${context.title}并切换`,
					description: '--switch',
					detail: `将从 ${GitReference.toString(state.reference)} 创建名为 ${state.name} 的新分支并切换到它`,
				}),
			],
			context,
		);
	...
	private async *deleteCommandSteps(state: DeleteStepState, context: Context): AsyncStepResultGenerator<void> {
		...
		while (this.canStepsContinue(state)) {
			...
			context.title = getTitle(
				state.references.length === 1 ? '分支' : `${state.references.length} 个分支`,
				state.subcommand,
			);
			...
			if (state.references.some(b => b.upstream != null)) {
				confirmations.push(
					FlagsQuickPickItem.create<DeleteFlags>(state.flags, ['--remotes'], {
						label: `${context.title}及远程跟踪分支`,
						description: '--remotes',
						detail: `将删除 ${GitReference.toString(state.references)} 及其远程跟踪分支`,
					}),
					FlagsQuickPickItem.create<DeleteFlags>(state.flags, ['--force', '--remotes'], {
						label: `强制${context.title}及远程跟踪分支`,
						description: '--force --remotes',
						detail: `将强制删除 ${GitReference.toString(state.references)} 及其远程跟踪分支`,
					}),
				);
			}
		}
